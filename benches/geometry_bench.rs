...
use eigen_rs::core::geometry::{EulerAngles, Ray, Hyperplane, AlignedBox};
...
fn bench_dot(c: &mut Criterion) {
    ...
    c.bench_function("vector3_dot", |b| b.iter(|| {
...
fn bench_cross(c: &mut Criterion) {
    ...
    c.bench_function("vector3_cross", |b| b.iter(|| {
...
fn bench_euler_conversion(c: &mut Criterion) {
    ...
    c.bench_function("euler_to_quaternion", |b| b.iter(|| {
        black_box(&ea).to_quaternion()
    }));
    
    let q = ea.to_quaternion();
    let mat = q.to_rotation_matrix();
    
    c.bench_function("rotation_matrix_to_euler", |b| b.iter(|| {
        EulerAngles::from_rotation_matrix(black_box(&mat))
    }));
}
...
fn bench_ray_aabb(c: &mut Criterion) {
    ...
    c.bench_function("ray_aabb_intersection", |b| b.iter(|| {
        black_box(&box_).intersects(black_box(&ray))
    }));
}

criterion_group!(benches, bench_dot, bench_cross, bench_euler_conversion, bench_ray_aabb);
