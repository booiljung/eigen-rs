...
fn spline_benchmark(c: &mut Criterion) {
    ...
    let points_vec = vec![0.0, 0.0, 1.0, 2.0, 2.0, 1.0, 3.0, 3.0, 4.0, 2.0];
    ...
    group.bench_function("Interpolate 5 points", |b| {
        b.iter(|| Spline::interpolate(black_box(&points), 3))
    });

    let spline = Spline::interpolate(&points, 3).unwrap();

    group.bench_function("Eval Spline", |b| b.iter(|| spline.eval(black_box(1.5))));
