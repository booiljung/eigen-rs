...
fn test_simd_sse_packet_eval() {
    ...
    unsafe { p2.store(res.as_mut_ptr()); }
    assert_eq!(res[0], 0.0);
    assert_eq!(res[1], 2.0);
    assert_eq!(res[2], 4.0);
    assert_eq!(res[3], 6.0);
...
#[cfg(target_arch = "x86_64")]
#[test]
fn test_simd_gemm_correctness() {
    use eigen_rs::core::ops::gemm::gemm_cm_packet;
    
    let mut a = Matrix::<f32, DynamicStorage<f32>>::new_dynamic(8, 8).unwrap();
    let mut b = Matrix::<f32, DynamicStorage<f32>>::new_dynamic(8, 8).unwrap();
    let mut c = Matrix::<f32, DynamicStorage<f32>>::new_dynamic(8, 8).unwrap();
    
    // Fill with predictable data
    for i in 0..8 {
        for j in 0..8 {
            *a.get_mut(i, j).unwrap() = i as f32;
            *b.get_mut(i, j).unwrap() = if i == j { 1.0 } else { 0.0 }; // Identity
        }
    }
    
    unsafe {
        gemm_cm_packet::<f32, _, _, _, SsePacketF32>(&a, &b, &mut c);
    }
    
    // c should be equal to a (since b is entry-wise identity)
    for i in 0..8 {
        for j in 0..8 {
            assert_eq!(*c.get(i, j).unwrap(), *a.get(i, j).unwrap());
        }
    }
}
