...
fn test_matrix_ops_differential() {
    ...
    *a.get_mut(0, 1).unwrap() = 20.0;
    *a.get_mut(1, 0).unwrap() = 30.0;
    *a.get_mut(1, 1).unwrap() = 40.0;

    *b.get_mut(0, 0).unwrap() = 1.0;
    *b.get_mut(0, 1).unwrap() = 2.0;
    *b.get_mut(1, 0).unwrap() = 3.0;
    *b.get_mut(1, 1).unwrap() = 4.0;
    ...
    for line in cpp_output.lines() {
        ...
        if parts.len() == 3 {
            let r: usize = parts[0]
                .trim()
                .parse()
                .unwrap_or_else(|_| panic!("Failed to parse row from line: '{}'", line));
            let c: usize = parts[1]
                .trim()
                .parse()
                .unwrap_or_else(|_| panic!("Failed to parse col from line: '{}'", line));
            let v_cpp: f32 = parts[2]
                .trim()
                .parse()
                .unwrap_or_else(|_| panic!("Failed to parse val from line: '{}'", line));
