    ...
    *m.get_mut(2, 0).unwrap() = -16.0; *m.get_mut(2, 1).unwrap() = -43.0; *m.get_mut(2, 2).unwrap() = 98.0;
    ...
    for line in cpp_output.lines() {
        ...
        if parts[0] == "LLT_L" {
            ...
            assert!((val_rust - val_cpp).abs() < 1e-5, "LLT L mismatch at {},{}", row, col);
        }
    }
}
...
fn test_llt_solve() {
    ...
    *a.get_mut(0, 0).unwrap() = 4.0; *a.get_mut(0, 1).unwrap() = 12.0; *a.get_mut(0, 2).unwrap() = -16.0;
    *a.get_mut(1, 0).unwrap() = 12.0; *a.get_mut(1, 1).unwrap() = 37.0; *a.get_mut(1, 2).unwrap() = -43.0;
    *a.get_mut(2, 0).unwrap() = -16.0; *a.get_mut(2, 1).unwrap() = -43.0; *a.get_mut(2, 2).unwrap() = 98.0;
    ...
    *b.get_mut(2, 0).unwrap() = 3.0;
    ...
    for line in cpp_output.lines() {
        ...
        if parts[0] == "LLT_SOLVE" {
            ...
            assert!((val_rust - val_cpp).abs() < 1e-5, "LLT solve mismatch at {}", idx);
        }
    }
}
