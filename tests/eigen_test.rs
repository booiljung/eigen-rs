...
use eigen_rs::core::decompositions::{Tridiagonalization, ComputationInfo};

#[test]
fn test_eigen_decomposition_3x3() {
    let mut m = MatrixX::<f32>::new_dynamic(3, 3).unwrap();
    // Symmetric matrix
    *m.get_mut(0, 0).unwrap() = 2.0; *m.get_mut(0, 1).unwrap() = 1.0; *m.get_mut(0, 2).unwrap() = 0.0;
    *m.get_mut(1, 0).unwrap() = 1.0; *m.get_mut(1, 1).unwrap() = 2.0; *m.get_mut(1, 2).unwrap() = 1.0;
    *m.get_mut(2, 0).unwrap() = 0.0; *m.get_mut(2, 1).unwrap() = 1.0; *m.get_mut(2, 2).unwrap() = 2.0;
    ...
    for i in 0..3 {
        ...
        for r in 0..3 {
            ...
            assert!((av[r] - lv).abs() < 1e-4, "Reconstruction failed at row {}, col {}: av={} lv={}", r, i, av[r], lv);
        }
    }
    ...
    for i in 0..3 {
        for j in 0..3 {
            ...
            if i == j {
                assert!((dot - 1.0).abs() < 1e-4, "Orthogonality failed at ({}, {}): dot={}", i, j, dot);
            } else {
                assert!(dot.abs() < 1e-4, "Orthogonality failed at ({}, {}): dot={}", i, j, dot);
            }
        }
    }
}
...
fn test_eigenvalues_2x2() {
    ...
    *m.get_mut(1, 0).unwrap() = 2.0; *m.get_mut(1, 1).unwrap() = 1.0;
    ...
    let (v1, v2) = if val1 < val2 { (val1, val2) } else { (val2, val1) };
...
fn test_tridiagonalization() {
    let mut m = MatrixX::<f32>::new_dynamic(3, 3).unwrap();
    *m.get_mut(0, 0).unwrap() = 4.0; *m.get_mut(0, 1).unwrap() = 1.0; *m.get_mut(0, 2).unwrap() = -2.0;
    *m.get_mut(1, 0).unwrap() = 1.0; *m.get_mut(1, 1).unwrap() = 3.0; *m.get_mut(1, 2).unwrap() = 0.0;
    *m.get_mut(2, 0).unwrap() = -2.0; *m.get_mut(2, 1).unwrap() = 0.0; *m.get_mut(2, 2).unwrap() = 5.0;
    ...
    for i in 0..3 {
        for j in 0..3 {
            if (i as isize - j as isize).abs() > 1 {
                assert!(t.get(i, j).unwrap().abs() < 1e-5, "T is not tridiagonal at ({}, {}): val={}", i, j, t.get(i, j).unwrap());
            }
        }
    }
    ...
    for i in 0..3 {
        for j in 0..3 {
            ...
            assert!((val - expected).abs() < 1e-4, "Reconstruction failed at ({}, {}): got {} expected {}", i, j, val, expected);
        }
    }
}
