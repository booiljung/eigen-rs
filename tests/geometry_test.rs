use eigen_rs::core::geometry::{AngleAxis, Quaternion, Transform3};
...
fn test_geometry_differential() {
    let cpp_output =
        common::run_cpp_harness_stdout("tests/cpp_harness/geometry_verify.cpp").unwrap();
        ...
        match parts[0] {
            ...
            "T_TRANS" => {
                ...
                *p.get_mut(2, 0).unwrap() = 30.0;
                ...
                assert!(
                    (p_trans.get(2, 0).unwrap() - z).abs() < 1e-5,
                    "T_TRANS fail at z: Rust={} C++={}",
                    p_trans.get(2, 0).unwrap(),
                    z
                );
            }
            _ => {}
