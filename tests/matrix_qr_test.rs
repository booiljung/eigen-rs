...
fn test_matrix_qr_differential() {
    ...
    *m.get_mut(2, 0).unwrap() = 5.0; *m.get_mut(2, 1).unwrap() = 6.0;
    ...
    for line in cpp_output.lines() {
        ...
        match parts[0] {
            "Q" => {
                ...
                assert!((v_rust.abs() - v_cpp.abs()).abs() < 1e-5, "Q abs mismatch at {},{}: Rust={} != C++={}", row, col, v_rust, v_cpp);
            ...
            "R" => {
                ...
                assert!((v_rust.abs() - v_cpp.abs()).abs() < 1e-5, "R abs mismatch at {},{}: Rust={} != C++={}", row, col, v_rust, v_cpp);
            },
            _ => {}
        }
    }
    ...
    let mut qr_prod = Matrix::<f32, eigen_rs::core::storage::DynamicStorage<f32>>::new_dynamic(3, 2).unwrap();
    ...
    for i in 0..3 {
        for j in 0..2 {
            ...
            assert!((v_orig - v_recon).abs() < 1e-5, "Reconstruction failed at {},{}: Orig={} Recon={}", i, j, v_orig, v_recon);
        }
    }
}
...
fn test_matrix_qr_orthogonality() {
    ...
    for i in 0..4 {
        for j in 0..4 {
            ...
            assert!((v - expected).abs() < 1e-5, "Orthogonality failed at {},{}: v={}", i, j, v);
        }
    }
}
