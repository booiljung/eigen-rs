...
fn test_matrix_inverse_differential() {
    ...
    *m4.get_mut(0, 0).unwrap() = 2.0; *m4.get_mut(0, 1).unwrap() = -1.0;
    *m4.get_mut(1, 0).unwrap() = -1.0; *m4.get_mut(1, 1).unwrap() = 2.0; *m4.get_mut(1, 2).unwrap() = -1.0;
    *m4.get_mut(2, 1).unwrap() = -1.0; *m4.get_mut(2, 2).unwrap() = 2.0; *m4.get_mut(2, 3).unwrap() = -1.0;
    *m4.get_mut(3, 2).unwrap() = -1.0; *m4.get_mut(3, 3).unwrap() = 2.0;
    ...
    for line in cpp_output.lines() {
        ...
        match parts[0] {
            "INV2" => {
                ...
                assert!((v_rust - v_cpp).abs() < 1e-5, "Inv2 mismatch at {},{}: Rust={} != C++={}", r, c, v_rust, v_cpp);
            },
            "INV4" => {
                let r: usize = parts[1].parse().unwrap();
                let c: usize = parts[2].parse().unwrap();
                ...
                assert!((v_rust - v_cpp).abs() < 1e-4, "Inv4 mismatch at {},{}: Rust={} != C++={}", r, c, v_rust, v_cpp);
            },
            _ => {}
        }
    }
...
fn test_matrix_inverse_identity() {
    ...
    *m.get_mut(0, 0).unwrap() = 2.0; *m.get_mut(1, 1).unwrap() = 4.0; *m.get_mut(2, 2).unwrap() = 8.0;
