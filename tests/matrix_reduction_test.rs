...
fn test_matrix_reductions_differential() {
    ...
    let cpp_output = common::run_cpp_harness_stdout("tests/cpp_harness/matrix_reduction_verify.cpp")
    ...
    for line in cpp_output.lines() {
        ...
        if parts.len() == 2 {
            ...
            match key {
                "SUM" => {
                    let v_rust = m.sum();
                    assert!((v_rust - v_cpp).abs() < 1e-5, "Sum mismatch: Rust={} != C++={}", v_rust, v_cpp);
                },
                "MIN" => {
                    let v_rust = m.min();
                    assert_eq!(v_rust, v_cpp, "Min mismatch: Rust={} != C++={}", v_rust, v_cpp);
                },
                "MAX" => {
                    let v_rust = m.max();
                    assert_eq!(v_rust, v_cpp, "Max mismatch: Rust={} != C++={}", v_rust, v_cpp);
                },
                "MEAN" => {
                    let v_rust = m.mean();
                    assert!((v_rust - v_cpp).abs() < 1e-5, "Mean mismatch: Rust={} != C++={}", v_rust, v_cpp);
                },
                _ => {}
            }
        }
