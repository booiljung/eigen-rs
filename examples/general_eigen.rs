...
use eigen_rs::decompositions::{EigenSolver, RealSchur, ComplexSchur, GeneralizedEigenSolver};
...
fn main() -> Result<(), String> {
    ...
    for i in 0..size { for j in 0..size { *a.get_mut(i, j).unwrap() = data_a[i*size+j]; } }
    ...
    let mut ac = Matrix::<Complex<f64>, DynamicStorage<Complex<f64>>>::new_dynamic(size, size).unwrap();
    for i in 0..size { for j in 0..size { *ac.get_mut(i, j).unwrap() = Complex::new(data_a[i*size+j], 0.0); } }
    ...
    if let Some(evecs) = esolver.eigenvectors() {
         let v0 = Matrix::<Complex<f64>, DynamicStorage<Complex<f64>>>::new_dynamic(size, 1).unwrap();
    ...
    let mut b = Matrix::<Complex<f64>, DynamicStorage<Complex<f64>>>::new_dynamic(size, size).unwrap();
     for i in 0..size { *b.get_mut(i, i).unwrap() = Complex::new(1.0, 0.0); } // Identity
