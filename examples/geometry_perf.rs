use eigen_rs::core::geometry::{Quaternion, Transform, Translation, Scaling};
...
fn main() {
    ...
    let q1 = { let mut q = q1; q.normalize(); q.conjugate() };
    
    let q2 = Quaternion::<f32>::new(0.4, 0.5, 0.6, 0.1);
    let q2 = { let mut q = q2; q.normalize(); q.conjugate() };

    // normalization check
    let mut q1 = Quaternion::<f32>::new(0.1, 0.2, 0.3, 0.9); q1.normalize();
    let mut q2 = Quaternion::<f32>::new(0.4, 0.5, 0.6, 0.1); q2.normalize();
    ...
    // Verify Correctness
    let diff = (res_scalar.x() - res_simd.x()).abs() + (res_scalar.y() - res_simd.y()).abs() + 
               (res_scalar.z() - res_simd.z()).abs() + (res_scalar.w() - res_simd.w()).abs();
    ...
    for i in 0..4 {
        for j in 0..4 {
            diff_tf += (*tf_scalar.matrix().get(i, j).unwrap() - *tf_simd.matrix().get(i, j).unwrap()).abs();
        }
    }
    ...
    for _ in 0..ITERATIONS {
        tp_scalar = t1.transform_point(&tp_scalar);
         // prevent explosion
         if tp_scalar.get(0,0).unwrap().abs() > 1e10 { tp_scalar.set_constant(1.0); }
    }
    ...
    if is_x86_feature_detected!("avx") && is_x86_feature_detected!("fma") {
        unsafe {
            for _ in 0..ITERATIONS {
                ...
                if tp_simd.get(0,0).unwrap().abs() > 1e10 { tp_simd.set_constant(1.0); }
            }
        }
    }
