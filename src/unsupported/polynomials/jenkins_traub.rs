...
impl<T: Scalar> JenkinsTraubSolver<T> {
    ...
    pub fn compute(&mut self, poly: &[T]) -> Result<(), String> {
        let n = poly.len();
        if n == 0 { return Err("Empty polynomial".to_string()); }
        ...
        let mut coeffs: Vec<Complex<T>> = poly.iter().map(|&c| Complex::new(c, T::zero())).collect();
    ...
    fn deflate(&self, p: &[Complex<T>], root: Complex<T>) -> Vec<Complex<T>> {
        ...
        let mut remainder = Complex::zero();
        for i in (0..n).rev() {
            let term = if i == n - 1 { p[i+1] } else { p[i+1] + remainder * root };
             // Wait, standard synthetic division:
             // Q coefficients from high to low.
             // q[n-1] corresponds to x^{n-1}.
             // p[n] corresponds to x^n.
             // q[n-1] = p[n]
             // q[k] = p[k+1] + q[k+1] * root
        }
