...
use crate::core::decompositions::lu::PartialPivLU;
use crate::core::matrix::{DynamicStorage, Matrix};
use crate::core::scalar::Scalar;
use crate::core::storage::Storage;
use crate::core::xpr::MatrixXpr; // Needed for solving D^-1 N
...
where
    T: Scalar<Real = T> + Copy + PartialOrd + StaticFloatConsts + 'static + num_traits::One,
    S: Storage<T> + 'static,
{
    ...
    pub fn compute(&self) -> Result<Matrix<T, DynamicStorage<T>>, String> {
        ...
        for r in 0..rows {
            for c in 0..cols {
                *o_poly.get_mut(r, c).unwrap() = T::default();
            }
        }
        for i in 0..rows {
            *o_poly.get_mut(i, i).unwrap() = c1;
        }
    ...
    fn add_scaled(
        &self,
        dest: &mut Matrix<T, DynamicStorage<T>>,
        src: &Matrix<T, DynamicStorage<T>>,
        scalar: T,
    ) {
        let rows = dest.rows();
        let cols = dest.cols();
    ...
    fn mul_matrix(
        &self,
        lhs: &Matrix<T, DynamicStorage<T>>,
        rhs: &Matrix<T, DynamicStorage<T>>,
    ...
    S: Storage<T> + 'static,
{
    ...
    pub fn compute(&self) -> Result<Matrix<T, DynamicStorage<T>>, String> {
        ...
        for i in 0..rows {
            for j in 0..cols {
                *a.get_mut(i, j).unwrap() = self.matrix.eval(i, j);
            }
        }
...
fn identity<T: Scalar>(rows: usize, cols: usize) -> Matrix<T, DynamicStorage<T>> {
    let mut m = Matrix::<T, DynamicStorage<T>>::new_dynamic(rows, cols)
        .expect("Failed to allocate identity matrix");
    for i in 0..rows {
        for j in 0..cols {
            let val = if i == j {
                T::from_usize(1)
