...
impl<T: Scalar + 'static> Spline<T> {
    ...
    pub fn new(knots: Vec<T>, ctrl_points: MatrixX<T>, degree: usize) -> Self {
        ...
        assert_eq!(knots.len(), n + degree + 1, "Invalid knot vector length: expected {}, got {}", n + degree + 1, knots.len());
    ...
    pub fn eval(&self, u: T) -> MatrixX<T> {
        ...
        for j in 0..=k {
            ...
            let mut m = MatrixX::new_dynamic(block.rows(), block.cols()).expect("Allocation failed");
    ...
    pub fn interpolate(points: &MatrixX<T>, degree: usize) -> Result<Self, String> {
        ...
        if n < degree + 1 {
            return Err(format!("Need at least degree + 1 points (got {}, degree {})", n, degree));
        }
