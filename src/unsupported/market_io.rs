...
pub fn load_matrix_market<P: AsRef<Path>, T: Scalar + FromStr>(path: P) -> Result<SparseMatrix<T>, String> 
where <T as FromStr>::Err: std::fmt::Display {
    ...
    // 1. Parse Header
    let first_line = lines.next().ok_or("Empty file")?.map_err(|e| e.to_string())?;
    ...
    // We need to consume lines until we find size
    // Iterating lines consumes them.
    let mut data_limit = 0; // count of data lines to read
    ...
    for line_res in lines.by_ref() {
        ...
        // This must be the size line
        let dims: Vec<&str> = trimmed.split_whitespace().collect();
        if dims.len() < 3 {
             return Err(format!("Invalid dimensions line: {}", line));
        }
        rows = dims[0].parse::<usize>().map_err(|_| "Invalid rows")?;
        cols = dims[1].parse::<usize>().map_err(|_| "Invalid cols")?;
        nnz = dims[2].parse::<usize>().map_err(|_| "Invalid nnz")?;
        data_limit = nnz;
        size_found = true;
        break;
    }
    ...
    if !size_found {
        return Err("Dimensions line not found".to_string());
    }
...
pub fn save_matrix_market<P: AsRef<Path>, T: Scalar + std::fmt::Display>(path: P, mat: &SparseMatrix<T>) -> Result<(), String> {
    ...
    writeln!(writer, "{} {} {}", mat.rows(), mat.cols(), triplet_count).map_err(|e| e.to_string())?;
