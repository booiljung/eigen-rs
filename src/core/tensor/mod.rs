...
impl<T: Scalar, const RANK: usize, D: Device> Tensor<T, RANK, D> {
    ...
    pub fn get_mut(&mut self, indices: [usize; RANK]) -> Option<&mut T> {
        ...
        self.storage.data_mut_opt().and_then(|data| data.get_mut(idx))
    }
    ...
    pub fn assign<X: xpr::TensorXpr<T, RANK>>(&mut self, xpr: &X) -> Result<(), String> {
        ...
        if let Some(data) = self.data_mut() {
            ...
            {
                // Try GPU execution
                if let (Some(device), Some(dest_storage)) = (self.device_as_cuda(), self.as_cuda_storage_mut()) {
    ...
    pub fn contract<const RANK2: usize, const OUT_RANK: usize>(
        ...
        if let Some(device) = self.device_as_cuda() {
            ...
            if RANK == 2 && RANK2 == 2 && lhs_dim == 1 && rhs_dim == 0 {
                ...
                let mut res = Tensor::<T, OUT_RANK, D>::new_with_device(out_dims, device_clone).expect("Failed to allocate result tensor");
                ...
                device.matmul(res_storage, lhs_storage, rhs_storage, m, n, k).unwrap();
        ...
        let contraction =
            contraction::TensorContraction::new(self, rhs, lhs_pair, rhs_pair);
        contraction.eval()
    }
        ...
        if let Some(dest_data) = res.data_mut() {
            ...
            for i in 0..size {
             ...
             if let (Some(device), Some(src_storage), Some(dest_storage)) = 
                (self.device_as_cuda(), self.as_cuda_storage(), res.as_cuda_storage_mut()) 
             {
                 // GPU Path
                 // Need input strides
             ...
             return Err("GPU Permute not implemented for this device or feature disabled".to_string());
        }
    ...
    pub fn reshape<const NEW_RANK: usize>(&self, new_dims: [usize; NEW_RANK]) -> Result<Tensor<T, NEW_RANK, D>, String> {
        let current_size = self.size();
        let new_size: usize = new_dims.iter().product();
        
        if current_size != new_size {
            return Err(format!("Reshape size mismatch: {} vs {}", current_size, new_size));
        }
        ...
        if let (Some(src), Some(dest)) = (self.data(), res.data_mut()) {
            dest.copy_from_slice(src);
             ...
             #[cfg(feature = "cuda")]
             if let (Some(src_storage), Some(dest_storage)) = (self.as_cuda_storage(), res.as_cuda_storage_mut()) {
                 dest_storage.copy_device_to_device(src_storage).map_err(|e| e)?;
                 return Ok(res);
             }
    ...
    #[cfg(feature = "cuda")]
    fn as_cuda_storage_mut(&mut self) -> Option<&mut crate::core::tensor::device::cuda::CudaStorage<T>> {
        use std::any::TypeId;
    ...
    fn device_as_cuda(&self) -> Option<&crate::core::tensor::device::cuda::CudaDevice> {
        ...
        if TypeId::of::<D>() == TypeId::of::<CudaDevice>() {
             unsafe {
                 Some(std::mem::transmute(&self.storage.device))
             }
        } else {
            None
        }
...
impl<T: Scalar, const RANK: usize> Tensor<T, RANK, CpuDevice> {
    ...
    where T: cust::memory::DeviceCopy
    {
        use crate::core::tensor::device::cuda::CudaStorage;
        
        let mut dest = Tensor::<T, RANK, crate::core::tensor::device::cuda::CudaDevice>::new_with_device(
            self.dims(), 
            device
        )?;
...
#[cfg(feature = "cuda")]
impl<T: Scalar + cust::memory::DeviceCopy, const RANK: usize> Tensor<T, RANK, crate::core::tensor::device::cuda::CudaDevice> {
    ...
    pub fn copy_from_host(&mut self, src: &[T]) -> Result<(), String> {
        self.storage.copy_from_host(src)
    }

    pub fn copy_to_host(&self, dest: &mut [T]) -> Result<(), String> {
        self.storage.copy_to_host(dest)
    }
}
...
impl<T: Scalar, const RANK: usize, D: Device> Tensor<T, RANK, D> {
    pub fn to_matrix(&self) -> Result<crate::core::matrix::MatrixX<T>, String> {
        if RANK != 2 {
            return Err(format!("to_matrix only supported for Rank 2 tensors, got {}", RANK));
        }
        let dims = self.dims();
    ...
    pub fn from_matrix_reshaped(mat: crate::core::matrix::MatrixX<T>, device: D, new_dims: [usize; RANK]) -> Result<Self, String> {
        let mat_size = mat.rows() * mat.cols();
        let tensor_size: usize = new_dims.iter().product();
        if mat_size != tensor_size {
            return Err(format!("Size mismatch in from_matrix_reshaped: Matrix {} != Tensor {}", mat_size, tensor_size));
        }
...
mod tests {
    ...
    fn test_tensor_multi_dim_contraction() {
        ...
        for x in t1.data_mut().unwrap() { *x = 1.0; }
        
        let mut t2 = Tensor::<f32, 3>::new([2, 2, 2]).unwrap();
        for x in t2.data_mut().unwrap() { *x = 2.0; }
...
mod cuda_tests {
    ...
    fn test_cuda_allocation() {
        if let Ok(_) = cust::quick_init() {
            let device = CudaDevice::default();
            let t = Tensor::<f32, 2, CudaDevice>::new_with_device([10, 10], device);
            if let Ok(t) = t {
    ...
    fn test_cuda_roundtrip() {
        if let Ok(_) = cust::quick_init() {
             let mut t_cpu = Tensor::<f32, 2>::new([2, 2]).unwrap();
             *t_cpu.get_mut([0,0]).unwrap() = 1.0;
             *t_cpu.get_mut([1,1]).unwrap() = 2.0;
             
             let device = CudaDevice::default();
             if let Ok(t_gpu) = t_cpu.to_device(device.clone()) {
                 let t_host_back = t_gpu.to_host().unwrap();
                 
                 assert_eq!(t_host_back.get([0,0]), Some(&1.0));
                 assert_eq!(t_host_back.get([1,1]), Some(&2.0));
             }
        }
    }
    ...
    fn test_cuda_add() {
        if let Ok(_) = cust::quick_init() {
             ...
             let mut t1_cpu = Tensor::<f32, 2>::new([2, 2]).unwrap();
             for x in t1_cpu.data_mut().unwrap() { *x = 1.0; }
             
             let mut t2_cpu = Tensor::<f32, 2>::new([2, 2]).unwrap();
             for x in t2_cpu.data_mut().unwrap() { *x = 2.0; }
             
             if let (Ok(t1_gpu), Ok(t2_gpu)) = (t1_cpu.to_device(device.clone()), t2_cpu.to_device(device.clone())) {
                 let mut t_res_gpu = Tensor::<f32, 2, CudaDevice>::new_with_device([2, 2], device.clone()).unwrap();
                 
                 // t_res = t1 + t2
                 t_res_gpu.assign(&(&t1_gpu + &t2_gpu)).unwrap();
                 
                 let t_res_cpu = t_res_gpu.to_host().unwrap();
                 
                 for x in t_res_cpu.data().unwrap() {
                     assert_eq!(*x, 3.0);
                 }
             }
        }
    ...
    fn test_cuda_matmul() {
        if let Ok(_) = cust::quick_init() {
             ...
             // 2x2 Identity
             let mut t1_cpu = Tensor::<f32, 2>::new([2, 2]).unwrap();
             *t1_cpu.get_mut([0,0]).unwrap() = 1.0;
             *t1_cpu.get_mut([1,1]).unwrap() = 1.0;
             
             let mut t2_cpu = Tensor::<f32, 2>::new([2, 2]).unwrap();
             for x in t2_cpu.data_mut().unwrap() { *x = 2.0; }
             
             if let (Ok(t1_gpu), Ok(t2_gpu)) = (t1_cpu.to_device(device.clone()), t2_cpu.to_device(device.clone())) {
                 // I * M = M. So result should be all 2.0 (if M is all 2.0)
                 // t2_cpu is all 2.0.
                 
                 let t3_gpu = t1_gpu.contract(&t2_gpu, 1, 0); // Rank 2 assumed output
                 
                 let t3_cpu = t3_gpu.to_host().unwrap();
                 
                 for x in t3_cpu.data().unwrap() {
                     assert_eq!(*x, 2.0);
                 }
             }
        }
    ...
    #[test]
    fn test_cuda_permute() {
        if let Ok(_) = cust::quick_init() {
             let device = CudaDevice::default();
             
             // 2x3 Tensor
             // ColMajor: [1, 2, 3, 4, 5, 6]
             // dims=[2, 3].
             // (0,0)->1, (1,0)->2
             // (0,1)->3, (1,1)->4
             // (0,2)->5, (1,2)->6
             let mut t_cpu = Tensor::<f32, 2>::new([2, 3]).unwrap();
             let data = t_cpu.data_mut().unwrap();
             data[0] = 1.0; data[1] = 2.0;
             data[2] = 3.0; data[3] = 4.0;
             data[4] = 5.0; data[5] = 6.0;
             ...
             if let Ok(t_gpu) = t_cpu.to_device(device.clone()) {
                 ...
                 let t_perm_gpu = t_gpu.permute([1, 0]).unwrap();
                 assert_eq!(t_perm_gpu.dims(), [3, 2]);
                 
                 let t_perm_cpu = t_perm_gpu.to_host().unwrap();
                 
                 assert_eq!(t_perm_cpu.get([0, 0]), Some(&1.0));
                 assert_eq!(t_perm_cpu.get([1, 0]), Some(&3.0));
                 assert_eq!(t_perm_cpu.get([2, 0]), Some(&5.0));
                 assert_eq!(t_perm_cpu.get([0, 1]), Some(&2.0));
                 assert_eq!(t_perm_cpu.get([1, 1]), Some(&4.0));
                 assert_eq!(t_perm_cpu.get([2, 1]), Some(&6.0));
             }
        }
    }

    #[test]
    fn test_cuda_reshape() {
         if let Ok(_) = cust::quick_init() {
             let device = CudaDevice::default();
             
             let mut t_cpu = Tensor::<f32, 2>::new([2, 3]).unwrap();
             for (i, x) in t_cpu.data_mut().unwrap().iter_mut().enumerate() {
                 *x = i as f32;
             }
             
             if let Ok(t_gpu) = t_cpu.to_device(device.clone()) {
                 let t_flat_gpu = t_gpu.reshape([6]).unwrap();
                 assert_eq!(t_flat_gpu.dims(), [6]);
                 
                 let t_flat_cpu = t_flat_gpu.to_host().unwrap();
                 
                 // Verify data consistency (0..6)
                 for (i, x) in t_flat_cpu.data().unwrap().iter().enumerate() {
