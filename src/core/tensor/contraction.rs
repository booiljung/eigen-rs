...
impl<'a, T: Scalar, const RANK1: usize, const RANK2: usize, D: Device>
    TensorContraction<'a, T, RANK1, RANK2, D>
{
    pub fn new(
        lhs: &'a Tensor<T, RANK1, D>,
        rhs: &'a Tensor<T, RANK2, D>,
    ...
    pub fn eval<const OUT_RANK: usize>(&self) -> Tensor<T, OUT_RANK, D> {
        ...
        assert_eq!(
            self.lhs_dims.len(),
            self.rhs_dims.len(),
            "Number of contraction dimensions must match"
        );
