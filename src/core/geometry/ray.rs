...
impl<T: Scalar, const RANK: usize> Ray<T, RANK> {
    pub fn new(origin: Matrix<T, FixedStorage<T, RANK, 1, RANK>>, direction: Matrix<T, FixedStorage<T, RANK, 1, RANK>>) -> Self {
    ...
    pub fn distance(&self, point: &Matrix<T, FixedStorage<T, RANK, 1, RANK>>) -> T {
        ...
        for i in 0..RANK {
            *diff.get_mut(i, 0).unwrap() = *point.get(i, 0).unwrap() - *self.origin.get(i, 0).unwrap();
        }
