...
impl<T: Scalar<Real = T> + PartialOrd, const N: usize, const DIM: usize, const SIZE: usize>
    Transform<T, N, DIM, SIZE>
{
    ...
    pub fn mul(&self, other: &Self) -> Self {
        #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
        {
            ...
            {
                unsafe {
                    let lhs_f32: &Transform<f32, 4, 3, 16> =
                        &*(self as *const Transform<T, N, DIM, SIZE>
                            as *const Transform<f32, 4, 3, 16>);
                    let rhs_f32: &Transform<f32, 4, 3, 16> = &*(other
                        as *const Transform<T, N, DIM, SIZE>
                        as *const Transform<f32, 4, 3, 16>);
                    let res = lhs_f32.mul_simd(rhs_f32);
                    return *(&res as *const Transform<f32, 4, 3, 16>
                        as *const Transform<T, N, DIM, SIZE>);
                }
            }
        }
                ...
                && is_x86_feature_detected!("fma")
            {
                unsafe {
                    ...
                    let lhs_f32: &Transform<f32, 4, 3, 16> =
                        &*(self as *const Transform<T, N, DIM, SIZE>
                            as *const Transform<f32, 4, 3, 16>);
                    let p_f32: &Matrix<f32, FixedStorage<f32, 3, 1, VSIZE>> =
                        &*(p as *const Matrix<T, FixedStorage<T, DIM, 1, VSIZE>>
                            as *const Matrix<f32, FixedStorage<f32, 3, 1, VSIZE>>);
                    let res = lhs_f32.transform_point_simd(p_f32);
                    return *(&res as *const Matrix<f32, FixedStorage<f32, 3, 1, VSIZE>>
                        as *const Matrix<T, FixedStorage<T, DIM, 1, VSIZE>>);
                }
            }
        }
