...
impl<T: Scalar> Quaternion<T> {
    ...
    pub fn identity() -> Self {
        Self::new(T::from_usize(0), T::from_usize(0), T::from_usize(0), T::from_usize(1))
    }
    ...
    pub fn x(&self) -> T { *self.coeffs.get(0, 0).unwrap() }
    pub fn y(&self) -> T { *self.coeffs.get(1, 0).unwrap() }
    pub fn z(&self) -> T { *self.coeffs.get(2, 0).unwrap() }
    pub fn w(&self) -> T { *self.coeffs.get(3, 0).unwrap() }

    pub fn coeffs(&self) -> &Vector4<T> { &self.coeffs }
    ...
    pub fn hamilton_product(&self, other: &Self) -> Self {
        let x1 = self.x(); let y1 = self.y(); let z1 = self.z(); let w1 = self.w();
        let x2 = other.x(); let y2 = other.y(); let z2 = other.z(); let w2 = other.w();
    ...
    pub fn rotate_vector(&self, v: &crate::core::matrix::Vector3<T>) -> crate::core::matrix::Vector3<T> {
        let q_v = Self::new(
            *v.get(0, 0).unwrap(),
            *v.get(1, 0).unwrap(),
            *v.get(2, 0).unwrap(),
        ...
        let res_q = self.hamilton_product(&q_v).hamilton_product(&self.conjugate());
