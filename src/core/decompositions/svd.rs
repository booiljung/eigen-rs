...
impl<T: Scalar + 'static, S: Storage<T> + 'static> JacobiSVD<T, S> {
    ...
    pub fn new(matrix: &Matrix<T, S>) -> Result<Self, String> {
        ...
        for i in 0..m {
            for j in 0..m {
                *u.get_mut(i, j).unwrap() = if i == j { T::from_usize(1) } else { T::from_usize(0) };
            }
        }
        ...
        for i in 0..n {
            for j in 0..n {
                *v.get_mut(i, j).unwrap() = if i == j { T::from_usize(1) } else { T::from_usize(0) };
            }
        }
                ...
                for j in i + 1..n {
                    ...
                    if a_ij.abs() > eps * (a_ii * a_jj).sqrt() {
                        ...
                        let t = if tau >= T::from_usize(0) {
                            ...
                            T::from_usize(0) - (T::from_usize(1) / ((T::from_usize(0) - tau) + (T::from_usize(1) + tau * tau).sqrt()))
                        };
    ...
    pub fn matrix_u(&self) -> &Matrix<T, DynamicStorage<T>> { &self.u }
    pub fn matrix_v(&self) -> &Matrix<T, DynamicStorage<T>> { &self.v }
    pub fn singular_values(&self) -> &[T] { &self.singular_values }
}
