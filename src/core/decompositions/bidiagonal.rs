...
impl<T: Scalar, S: Storage<T>> Bidiagonalization<T, S> {
    ...
    pub fn new(matrix: &Matrix<T, S>) -> Result<Self, String> {
        ...
        if is_upper {
            Self::compute_upper(&mut packed_matrix, &mut householder_coeffs_left, &mut householder_coeffs_right);
        } else {
             Self::compute_lower(&mut packed_matrix, &mut householder_coeffs_left, &mut householder_coeffs_right);
        }
    ...
    fn compute_upper(mat: &mut Matrix<T, DynamicStorage<T>>, h_left: &mut Vec<T>, h_right: &mut Vec<T>) {
        let rows = mat.rows();
        let cols = mat.cols();
        ...
        for k in 0..cols {
            ...
            if k + 1 < cols { // Need at least 2 elements to reflect
                 ...
                 if row_norm.abs() > T::epsilon() {
                     let v0 = val_k1;
                     let sigma = if v0 >= T::default() { -row_norm } else { row_norm };
                     let v0_s = v0 - sigma;
    ...
    fn compute_lower(mat: &mut Matrix<T, DynamicStorage<T>>, _h_left: &mut Vec<T>, _h_right: &mut Vec<T>) {
         // Placeholder
    ...
    pub fn matrix_u(&self) -> Matrix<T, DynamicStorage<T>> {
        ...
        for i in 0..m {
            for j in 0..m {
                *u.get_mut(i, j).unwrap() = if i == j { T::from_usize(1) } else { T::default() };
            }
        }
        ...
        for k in (0..num_h).rev() {
            ...
            if tau != T::default() {
                ...
                for j in k..m { // For each column of U (only k..m affected)
    ...
    pub fn matrix_v(&self) -> Matrix<T, DynamicStorage<T>> {
        ...
        for i in 0..n {
            for j in 0..n {
                *v.get_mut(i, j).unwrap() = if i == j { T::from_usize(1) } else { T::default() };
            }
        }
        ...
        for k in (0..num_p).rev() {
            ...
            if tau != T::default() {
                ...
                for j in k+1..n { // For each column j of V
...
mod tests {
    ...
    fn test_bidiagonal_upper() -> Result<(), String> {
        ...
        for i in 0..rows {
            for j in 0..cols {
                ...
                let is_super = i + 1 == j;
                if !is_diag && !is_super {
                    assert!(b.get(i, j).unwrap().abs() < 1e-9, "Non-zero at ({}, {}) for B", i, j);
                }
            }
        }
        ...
        for i in 0..rows {
            for j in 0..cols {
                let mut sum = 0.0;
                for k in 0..rows { // B has rows=4
                     // But B is effective only 4x3?
                     // B is 4x3. U is 4x4.
                     if k < cols { // optimization, B(:, k >= cols) are zero.
        ...
        for i in 0..rows {
            for j in 0..cols {
                 assert!((res.get(i, j).unwrap() - a.get(i, j).unwrap()).abs() < 1e-9, 
                     "Mismatch at ({}, {}): {} != {}", i, j, res.get(i, j).unwrap(), a.get(i, j).unwrap());
            }
        }
    ...
    fn test_bidiagonal_orthogonality() -> Result<(), String> {
        ...
        use crate::core::matrix::MatrixX; // Re-use standard matrix ops if possible or just use fully qualified imports? 
        // We are in core::decompositions::bidiagonal, so crate::core::matrix::Matrix is available.
        // We need to import the multiplication traits? They are in scope of Matrix usually?
        // Actually, assign_product takes &Product.
        // let prod = &ut * &u; 
        // But u is moved? u.transpose() borrows u.
        // u is Matrix. &u is &Matrix.
        // &ut * &u works.
        ...
        for i in 0..rows {
            for j in 0..rows {
                ...
                assert!((utu.get(i, j).unwrap() - expected).abs() < 1e-9, "U Orthogonality failed at {},{}", i, j);
            }
        }
