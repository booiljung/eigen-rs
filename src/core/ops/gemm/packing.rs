    ...
    if mr == 16 && rs == 1 {
        ...
        for i in (0..mc).step_by(mr) {
            ...
            for k in 0..kc {
                ...
                if mr_eff == 16 {
                    // Fast path: Memcpy 16 floats
                    std::ptr::copy_nonoverlapping(a_ptr_k, packed_ptr, 16);
                } else {
                    // Partial pack
                    for r in 0..mr_eff {
...
pub unsafe fn pack_rhs_f32_avx(
    ...
    // Optimized for NR=6, Col-Major B (rs=1).
    if nr == 6 && rs == 1 {
        use std::arch::x86_64::*;
        let mut packed_ptr = packed;
        ...
        for j in (0..nc).step_by(nr) {
            ...
            if nr_eff == 6 {
                 ...
                 while k + 4 <= kc {
                     ...
                     let b4_ptr = b.offset((k as isize) + (j as isize + 4) * cs);
                     let b5_ptr = b.offset((k as isize) + (j as isize + 5) * cs);
                     
                     let v0 = _mm_loadu_ps(b0_ptr); 
                     let v1 = _mm_loadu_ps(b1_ptr); 
                     let v2 = _mm_loadu_ps(b2_ptr);
                     let v3 = _mm_loadu_ps(b3_ptr);
                     let v4 = _mm_loadu_ps(b4_ptr);
                     let v5 = _mm_loadu_ps(b5_ptr);
                     
                     let mut t0 = v0; let mut t1 = v1; let mut t2 = v2; let mut t3 = v3;
                     _MM_TRANSPOSE4_PS(&mut t0, &mut t1, &mut t2, &mut t3);
                     
                     let t4 = _mm_unpacklo_ps(v4, v5); 
                     let t5 = _mm_unpackhi_ps(v4, v5); 
                     
                     _mm_storeu_ps(packed_ptr, t0);
                     _mm_store_sd(packed_ptr.add(4) as *mut f64, _mm_castps_pd(t4)); 
                     packed_ptr = packed_ptr.add(6);
                     
                     _mm_storeu_ps(packed_ptr, t1);
                     _mm_storeh_pd(packed_ptr.add(4) as *mut f64, _mm_castps_pd(t4));
                     packed_ptr = packed_ptr.add(6);
                     
                     _mm_storeu_ps(packed_ptr, t2);
                     _mm_store_sd(packed_ptr.add(4) as *mut f64, _mm_castps_pd(t5));
                     packed_ptr = packed_ptr.add(6);
                     
                     _mm_storeu_ps(packed_ptr, t3);
                     _mm_storeh_pd(packed_ptr.add(4) as *mut f64, _mm_castps_pd(t5));
                     packed_ptr = packed_ptr.add(6);
                 ...
                 for kk in k..kc {
                     for c in 0..6 {
                         *packed_ptr.add(c) = *b.offset((kk as isize) + (j as isize + c as isize) * cs);
                     }
                     packed_ptr = packed_ptr.add(6);
                 }
                 ...
                 for k in 0..kc {
                     for c in 0..nr_eff {
                         *packed_ptr.add(c) = *b.offset((k as isize) + (j as isize + c as isize) * cs);
                     }
                     for c in nr_eff..nr {
