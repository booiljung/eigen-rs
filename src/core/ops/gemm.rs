...
use self::packing::{pack_lhs, pack_rhs};
...
pub unsafe fn gemm_blocked<T, K>(
    ...
    a: *const T, rs_a: isize, cs_a: isize,
    b: *const T, rs_b: isize, cs_b: isize,
    c: *mut T,   rs_c: isize, cs_c: isize
) -> Result<(), String>
where
    ...
    K: GemmKernel<Elem = T>,
{
    ...
    // MC x KC
    let packed_a_len = MC * KC + MC; 
    let mut packed_a = vec![T::default(); packed_a_len];

    // KC x NC
    let packed_b_len = KC * NC + NC;
        ...
        for pc in (0..k).step_by(KC) {
            ...
            for ic in (0..m).step_by(MC) {
                ...
                for jr in (0..nc_eff).step_by(K::NR) {
                    let nr_eff = std::cmp::min(nc_eff - jr, K::NR);

                    for ir in (0..mc_eff).step_by(K::MR) {
                        let mr_eff = std::cmp::min(mc_eff - ir, K::MR);
                        ...
                        // C(ic+ir, jc+jr)
                        let c_ptr = c.offset((ic as isize + ir as isize) * rs_c + (jc as isize + jr as isize) * cs_c);
...
pub fn gemm_cm_unoptimized_xpr<T, L, R, SC>(a: &L, b: &R, c: &mut Matrix<T, SC>) -> Result<(), String>
where
    T: Scalar,
    L: MatrixXpr<T>,
    R: MatrixXpr<T>,
...
pub fn gemm_cm<T, SA, SB, SC>(a: &Matrix<T, SA>, b: &Matrix<T, SB>, c: &mut Matrix<T, SC>) -> Result<(), String>
where
