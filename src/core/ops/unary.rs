...
impl<T: Scalar> UnaryFunctor<T> for ScalarSin {
    fn eval(&self, x: T) -> T { x.sin() }
    fn packet_eval<P: Packet<T>>(&self, x: P) -> P { x.psin() }
}

pub struct ScalarCos;
impl<T: Scalar> UnaryFunctor<T> for ScalarCos {
    fn eval(&self, x: T) -> T { x.cos() }
    fn packet_eval<P: Packet<T>>(&self, x: P) -> P { x.pcos() }
}

pub struct ScalarExp;
impl<T: Scalar> UnaryFunctor<T> for ScalarExp {
    fn eval(&self, x: T) -> T { x.exp() }
    fn packet_eval<P: Packet<T>>(&self, x: P) -> P { x.pexp() }
}

pub struct ScalarLog;
impl<T: Scalar> UnaryFunctor<T> for ScalarLog {
    fn eval(&self, x: T) -> T { x.ln() }
    fn packet_eval<P: Packet<T>>(&self, x: P) -> P { x.plog() }
}
