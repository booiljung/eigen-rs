...
pub mod sse_f32 {
    ...
    unsafe fn set1(v: f32) -> __m128 { unsafe { _mm_set1_ps(v) } }
    ...
    pub unsafe fn psin(x_in: __m128) -> __m128 { 
        unsafe {
            ...
            let x = _mm_add_ps(_mm_add_ps(_mm_add_ps(abs_x, 
                                        _mm_mul_ps(y, set1(DP1))),
            ...
            let combined_sign = _mm_xor_ps(sign_mask, _mm_andnot_ps(sign_flip_mask, _mm_set1_ps(-0.0)));
