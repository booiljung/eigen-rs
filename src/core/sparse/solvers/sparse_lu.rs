    ...
    pub fn factorize(&mut self, matrix: &SparseMatrix<T>) -> Result<(), String> {
        ...
        for j in 0..n {
            for &(i_idx, val) in &l_cols[j] {
                l_triplets.push(crate::core::sparse::sparse_matrix::Triplet::new(i_idx, j, val));
            }
            for &(i_idx, val) in &u_cols[j] {
                u_triplets.push(crate::core::sparse::sparse_matrix::Triplet::new(i_idx, j, val));
            }
        }
...
mod tests {
    ...
    fn test_sparse_lu_basic() {
        ...
        a.set_from_triplets(vec![
            Triplet::new(0, 0, 1.0), Triplet::new(0, 1, 2.0),
            Triplet::new(1, 1, 3.0), Triplet::new(1, 2, 4.0),
            Triplet::new(2, 0, 5.0), Triplet::new(2, 2, 6.0),
        ]);
