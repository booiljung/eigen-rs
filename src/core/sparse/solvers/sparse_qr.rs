...
use crate::core::sparse::ordering::{Ordering, NaturalOrdering, Permutation};
        ...
        for k in 0..n {
             ...
             if k < size {
                 ...
                 if norm != T::zero() {
                     let v0 = workspace[k];
                     // Stable sign choice
                     let sigma = if v0 >= T::zero() { norm } else { T::zero() - norm };
                     let v0_new = v0 + sigma;
                 ...
                 for i in 0..size { // Only up to size rows in R? Usually R is m x n but zero below diagonal?
