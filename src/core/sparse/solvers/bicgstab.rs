        ...
        for k in 0..b.cols() {
            ...
            for iter in 0..self.max_iterations {
                ...
                if iter == 0 {
                    ...
                    // p = r + beta * (p - omega * v)
                    for i in 0..n {
                        *p.get_mut(i, 0).unwrap() = *r.get(i, 0).unwrap() + beta * (*p.get(i, 0).unwrap() - omega * *v.get(i, 0).unwrap());
                    }
                }
                ...
                let mut s = r.clone();
                for i in 0..n {
                    *s.get_mut(i, 0).unwrap() = *s.get(i, 0).unwrap() - alpha * *v.get(i, 0).unwrap();
                }
                
                if s.norm_f64() / b_norm < self.tolerance {
                    // x = x + alpha * p_hat
                    for i in 0..n {
                        *x.get_mut(i, k).unwrap() = *x.get(i, k).unwrap() + alpha * *p_hat.get(i, 0).unwrap();
                    }
                    break;
                }
                ...
                for i in 0..n {
                    *x.get_mut(i, k).unwrap() = *x.get(i, k).unwrap() + alpha * *p_hat.get(i, 0).unwrap() + omega * *s_hat.get(i, 0).unwrap();
                }
                
                // r = s - omega * t
                for i in 0..n {
                    *r.get_mut(i, 0).unwrap() = *s.get(i, 0).unwrap() - omega * *t.get(i, 0).unwrap();
                }
