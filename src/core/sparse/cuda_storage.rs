...
impl<T: Scalar> CudaSparseStorage<T> {
    pub fn new(rows: usize, cols: usize, nnz: usize) -> Result<Self, String> {
        ...
        unsafe {
            ...
            let res = cudaMalloc(
                &mut values_ptr as *mut *mut T as *mut *mut std::ffi::c_void,
            ...
            let res = cudaMalloc(
                &mut col_ptr as *mut *mut i32 as *mut *mut std::ffi::c_void,
            ...
            let res = cudaMalloc(
                &mut row_ptr as *mut *mut i32 as *mut *mut std::ffi::c_void,
    ...
    pub fn copy_from_host(
        ...
        unsafe {
            ...
            if res != cudaError_t::Success {
                return Err("H2D Copy failed (values)".to_string());
            }
            ...
            if res != cudaError_t::Success {
                return Err("H2D Copy failed (col_indices)".to_string());
            }
            ...
            if res != cudaError_t::Success {
                return Err("H2D Copy failed (row_offsets)".to_string());
            }
        }
        Ok(())
    }
